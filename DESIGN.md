...
429/503 (three attempts by default), and an automatic query-scoped completion ledger under
...
| HTTPS `Fetcher` (HTTPS-only enforced except for an exact, opt-in, always-warned `http://oai.bnf.fr/oai2/OAIHandler` exception; std TLS verify; **honest identifying UA** by default with per-host browser-spoof override only where forced e.g. Gallica; honest `Accept`; status mapping; **rejects HTML interstitials** so a bot-wall/error page is never archived; narrow LOC manifest-403 → official item-JSON derived-manifest fallback) | ✅ done | `internal/source` |
...
| Read-only library doctor (`-doctor`) validates bundle JSON/provenance, referenced images, every full-size and tiled JPEG advertised by local `info.json`, annotations, unsafe paths, and catalogue consistency; warnings are non-fatal, integrity errors return non-zero; incomplete linked resources produce deduplicated, shell-safe manifest rerun commands | ✅ done | `internal/serve`, `cmd/iiifpreserve` |
