...
| Two-to-four manuscript comparison: accessible ordered catalogue tray, bookmarkable validated local-slug URL, Mirador mosaic windows, and strict Presentation 2/3 canvas ownership routing to per-bundle annotation adapters; ambiguous/unowned canvases fail closed without cross-bundle writes | ✅ slices 1–2 | `internal/serve`, `viewer-src` |
...
| Researcher metadata exchange (`-export-metadata FILE` / `-import-metadata FILE`) — portable versioned JSON contains only catalogue overrides/notes/tags plus W3C annotations. Import matches original manifest URL across differing layouts and is non-destructive: fills blank title/notes, unions tags, adds new annotation IDs, ignores exact duplicates, warns and keeps local conflicts/missing bundles. `-dry-run` previews counts/warnings without writes; atomic catalogue/annotation writes and a library-wide process lock prevent concurrent server/import updates (the server must be stopped for an actual import) | ✅ done | `internal/serve`, `cmd/iiifpreserve` |
