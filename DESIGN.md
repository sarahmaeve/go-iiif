...
| **Known gap:** point-and-drag region selection on the image (draw a highlight / anchor a note to a spot) needs in-canvas drawing — only achievable via a vendored Mirador+MAE bundle (Option A). The pure-Go chrome supports numeric `xywh` regions only. | ⬜ Option A (decide) | DESIGN §C |
