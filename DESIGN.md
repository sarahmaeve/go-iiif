...
| **Offline annotation store (D)** — user W3C Web Annotations kept beside the bundle as `annotations.json` (AnnotationPage; atomic writes); serve-time injection attaches them to the targeted Canvas (v2+v3, inline items, no extra fetch) so Mirador displays them offline. Zero-impact when absent; never breaks serving. **Authoring (C1):** `POST /<dir>/annotations` appends a client W3C annotation to the store (loopback-only, single-user, no auth — same trust model as serving; mints an id if absent; rejects non-bundle paths / bad JSON / no target) → displayed via the existing injection. **Authoring UI (C2):** an editorial `<details>` panel in the viewer masthead (note / translation / tag / highlight / bookmark; page chooser — sequence-numbered, "NP"/blank labels suppressed; optional xywh region) POSTs to C1 — pure dependency-free JS, decoupled from Mirador internals. **Version-correct display:** v3 → `canvas.annotations` (W3C AnnotationPage); v2 → `canvas.otherContent` (`sc:AnnotationList`/`oa:Annotation`, W3C→OA at serve time) — the shape the shipped Mirador bundle actually reads per version (v2 ignores the P3 key). | ✅ done | `internal/annotation`, `internal/serve` |
