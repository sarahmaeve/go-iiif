...
| Embedded **Mirador 4** viewer (`go:embed` UMD; `/<dir>/` + bundle route); **rich index** — per-manifest title / language / institution (links to the IIIF record) / ~pages / ~size from stored manifest+provenance+disk, no re-fetch; styled in the "Literary Longform" editorial design with **vendored OFL fonts served locally** (Newsreader/Source Serif 4/IBM Plex Mono, `go:embed`, offline — no CDN); per-manuscript viewer carries a quiet editorial masthead (kicker, title, source→IIIF-record, ← Catalogue) above full-bleed Mirador | ✅ done | `internal/serve` |
| **Offline annotation store (D)** — user W3C Web Annotations kept beside the bundle as `annotations.json` (AnnotationPage; atomic writes); serve-time injection attaches to the targeted Canvas a **reference** to a per-canvas annotation endpoint on this same loopback server — because Mirador loads annotations by *fetching* the AnnotationPage/AnnotationList by id (it ignores inline items/resources; confirmed by reading the shipped bundle). v3 → `canvas.annotations` (`?fmt=w3c` → W3C AnnotationPage); v2 → `canvas.otherContent` (`?fmt=oa` → `sc:AnnotationList`/`oa:Annotation`, W3C→OA at serve time). Still fully offline (endpoint is local). Zero-impact when absent; never breaks serving. **Authoring (C1):** `POST /<dir>/annotations` appends a client W3C annotation to the store (loopback-only, single-user, no auth — same trust model as serving; mints an id if absent; rejects non-bundle paths / bad JSON / no target) → displayed via the existing injection. **Authoring UI (C2):** an editorial `<details>` panel in the viewer masthead (note / translation / tag / highlight / bookmark; page chooser — sequence-numbered, "NP"/blank labels suppressed; optional xywh region) POSTs to C1 — pure dependency-free JS, decoupled from Mirador internals. The annotation endpoint (`GET /<dir>/annotations`) doubles as the MAE storage-adapter backend (`?canvas=`/`?fmt=` for display; no params = full AnnotationPage = adapter `all`). | ✅ done | `internal/annotation`, `internal/serve` |
...
| **Option A frontend (remaining):** vendor a Node-built Mirador 4 + MAE UMD bundle + a ~30-line JS storage adapter mapping MAE → the REST endpoints, for in-canvas point-and-drag region selection (highlight / anchor a note to a spot). Pure-Go chrome stays as the fallback (numeric `xywh` only). | ⬜ decide/next | DESIGN §C |
